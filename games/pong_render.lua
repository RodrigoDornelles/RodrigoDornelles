local function init(std, game)
    ...
    local img = canvas:new(64, 64)

    canvas.attrColor(img, 'blue')
    canvas.drawRect(img, 'fill', 0, 0, 8, 8)
    canvas.attrColor(img, 'red')
    canvas.drawRect(img, 'fill', 4, 4, 4, 4)
    game.ball = img
end
...
local function draw(std, game)
    ...
    canvas:compose(game.ball_pos_x, game.ball_pos_y, game.ball)
